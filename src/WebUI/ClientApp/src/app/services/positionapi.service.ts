...
export class PositionApiService {
  ...
  public update(position: PositionEdit): Observable<any> {
    return this.http.put(
      this.positionApiRoot + '/' + position.positionID,
      position
    );
  }
  ...
  public replace(
    positionID: number,
    date: string,
    personsToDismiss: number[],
    personsToAssign: number[]
  ): Observable<any> {
    console.log(
      `assigning ${personsToAssign.join(
        ', '
      )}, dismissing ${personsToDismiss.join(
        ', '
      )} to ${positionID} for date ${date}`
    );
    return from([null]);
  }
